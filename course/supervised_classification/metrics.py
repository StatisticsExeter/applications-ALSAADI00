...
def metric_report(y_test_path, y_pred_path, report_path):
    ...
    # Create classification report as a dict, then to DataFrame
    report_dict = classification_report(y_test, y_pred, output_dict=True)
    report = pd.DataFrame(report_dict)
    # Ensure output directory exists
    report_path.parent.mkdir(parents=True, exist_ok=True)
    # Save transposed report (labels as rows)
...
def metric_report_logreg():
    base_dir = find_project_root()
    y_pred_path = base_dir / "data_cache" / "models" / "logreg_y_pred.csv"
    y_test_path = base_dir / "data_cache" / "energy_y_test.csv"
    report_path = base_dir/ 'data_cache' / 'vignettes' / 'supervised_classification' / 'logreg.csv'
    metric_report(y_test_path, y_pred_path, report_path)
